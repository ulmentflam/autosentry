---
description: Check whether autosentry is behind and upgrade with the right backend (uv / pipx / pip / Homebrew).
argument-hint: "[--pre|--version <x.y.z>|--method <uv|pipx|pip>]"
---

# /autosentry-update — Pi
...
3. Behind → run the recommended command (ask first): Homebrew →
   ...
   `update` auto-detects the backend (`--method` forces it,
   `--version` pins a release, `--pre` allows pre-releases).

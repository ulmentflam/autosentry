...
Most supervisors handle (1). Almost nothing handles (3) without a human
in the chair. autosentry handles all three: deterministic YAML rules
for the boring cases, an LLM (Claude Code by default) for the
open-ended ones, and a structured incident store so a real human can
audit either path later. The "self-healing" label is honest for (1)
and most of (2); for (3) it's best-effort — the fix-branch +
verification flow exists precisely because LLM patches sometimes make
things worse.
...
outcomes. Silent is healthy. If `autosentry status` shows
`state=running` and `restarts=0`, you're good — open
`autosentry watch` for a live view, or `autosentry doctor` if anything
looks off.
...
| `healing.escalate_to_claude_after` | int | half of `max_restarts`       | force-escalate to Claude after N unverified restarts |
...
satisfied by a detection wins. Anything unmatched falls through to the
Claude healer, which runs in one of two modes — picked automatically
based on what's installed (see [mode resolution](#healer-modes) below).
...
The restart counter is **outcome-aware**, not a dumb tally. Two pieces:
...
- **Force-escalate before exhaustion.** Once `state.restarts` hits
  `healing.escalate_to_claude_after` (defaults to half of
  `max_restarts`), the *next* detection skips the rule healer and goes
  straight to Claude. The heavier diagnosis gets a shot **before** the
  budget runs out on restarts that aren't sticking.
...
No. With `healing.claude.enabled: false` (or simply with no `claude`
CLI on PATH when `enabled: auto`), autosentry is rules-only. `doctor`
won't redline — rules-only is a first-class mode, not a degraded one.
Useful when your rule set is well-tested or you're on a machine
